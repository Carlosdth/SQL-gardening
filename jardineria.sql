...
SELECT codigo_pedido, codigo_cliente, fecha_esperada, fecha_entrega
FROM pedido
WHERE fecha_esperada = date_add(fecha_entrega, interval 2 day);
...
SELECT empleado.nombre, jefe.nombre as nombre_del_jefe
FROM empleado      
LEFT JOIN empleado AS jefe
	ON jefe.codigo_empleado = empleado.codigo_jefe;

-- 25. Devuelve un listado que muestre el nombre de cada empleado, el nombre de su jefe y el nombre del jefe de sus jefe.

SELECT empleado.nombre, jefe.nombre AS nombre_del_jefe, jefe_jefe.nombre AS jefe_del_jefe
FROM empleado      
LEFT JOIN empleado AS jefe
	ON jefe.codigo_empleado = empleado.codigo_jefe
LEFT JOIN empleado AS jefe_jefe
	ON jefe_jefe.codigo_empleado = jefe.codigo_jefe;
...
FROM (SELECT cl.codigo_cliente, cl.nombre_cliente, pe.codigo_pedido
	FROM cliente AS cl
	INNER JOIN pedido AS pe
		ON cl.codigo_cliente = pe.codigo_pedido
	GROUP BY cl.nombre_cliente) AS t1
INNER JOIN (SELECT det.codigo_pedido, pr.gama
	FROM producto AS pr
	INNER JOIN detalle_pedido AS det
		ON pr.codigo_producto = det.codigo_producto) AS t2
...
SELECT pago1.nombre_cliente
FROM (SELECT cl.nombre_cliente, cl.codigo_cliente
	FROM cliente AS cl
	LEFT JOIN pago AS p
		ON cl.codigo_cliente = p.codigo_cliente
	WHERE fecha_pago IS NULL) AS pago1
INNER JOIN (SELECT cl.nombre_cliente, cl.codigo_cliente
	FROM cliente AS cl
	LEFT JOIN pedido AS p
		ON cl.codigo_cliente = p.codigo_cliente
	WHERE fecha_pedido IS NULL) AS pedido1
    ON pago1.codigo_cliente = pedido1.codigo_cliente;
    
-- 31. Devuelve un listado que muestre solamente los empleados que no tienen una oficina asociada.

SELECT em.nombre, o.ciudad
FROM empleado AS em
...
SELECT empleado1.nombre, oficina.*
FROM (SELECT em.nombre, em.codigo_oficina
	FROM cliente AS cl
	RIGHT JOIN empleado AS em
		ON em.codigo_empleado = cl.codigo_empleado_rep_ventas
	WHERE nombre_cliente IS NULL) AS empleado1
INNER JOIN oficina
	ON empleado1.codigo_oficina = oficina.codigo_oficina;
...
SELECT t1.nombre AS no_cliente, t2.nombre AS no_oficina
FROM (SELECT em.codigo_empleado, em.nombre
	FROM cliente AS cl
	RIGHT JOIN empleado AS em
		ON em.codigo_empleado = cl.codigo_empleado_rep_ventas
	WHERE nombre_cliente IS NULL) AS t1
LEFT JOIN (SELECT em.codigo_empleado, em.nombre
	FROM empleado AS em
	LEFT JOIN oficina AS ofi
		ON em.codigo_oficina = ofi.codigo_oficina
	WHERE ofi.codigo_oficina IS NULL) AS t2
	ON t1.codigo_empleado = t2.codigo_empleado;
...
LEFT JOIN detalle_pedido AS det
	ON p.codigo_producto = det.codigo_producto
WHERE det.cantidad IS NULL;
...
SELECT t2.nombre, t2.descripcion, t1.imagen, t2.cantidad
FROM (SELECT p.codigo_producto, p.nombre, g.imagen
	FROM producto AS p
	INNER JOIN gama_producto AS g
		ON p.gama = g.gama) AS t1
RIGHT JOIN (SELECT p.codigo_producto, p.descripcion, det.cantidad, p.nombre
	FROM producto AS P
	LEFT JOIN detalle_pedido AS det
		ON p.codigo_producto = det.codigo_producto
	WHERE det.cantidad IS NULL) AS t2
...
SELECT t3.codigo_oficina, t3.ciudad
FROM (SELECT em.codigo_empleado, ofi.codigo_oficina, ofi.ciudad
	FROM empleado em
	INNER JOIN oficina ofi
		ON em.codigo_oficina = ofi.codigo_oficina) AS t3
LEFT JOIN (SELECT t1.gama, t1.codigo_pedido, t2.nombre_cliente, t2.codigo_empleado_rep_ventas 
	FROM (SELECT p.gama, det.codigo_pedido
		FROM producto p
		INNER JOIN detalle_pedido det
			ON p.codigo_producto = det.codigo_producto) AS t1
	INNER JOIN (SELECT p.codigo_pedido, cl.nombre_cliente, cl.codigo_empleado_rep_ventas
		FROM cliente cl
		INNER JOIN pedido p
			ON cl.codigo_cliente = p.codigo_cliente) AS t2
    ON t1.codigo_pedido = t2.codigo_pedido
	WHERE gama = 'Frutales') AS t4
	ON t3.codigo_empleado = t4.codigo_empleado_rep_ventas
WHERE gama IS NULL
GROUP BY t3.codigo_oficina;

-- 38. Devuelve un listado con los clientes que han realizado algún pedido pero no han realizado ningún pago.

SELECT t1.codigo_cliente, t1.nombre_cliente
FROM (SELECT cl.codigo_cliente, cl.nombre_cliente, p.fecha_pedido
	FROM cliente cl
	INNER JOIN pedido p
		ON cl.codigo_cliente = p.codigo_cliente) AS t1
LEFT JOIN pago p
	ON t1.codigo_cliente = p.codigo_cliente
...
SELECT e.nombre, t1.nombre_del_jefe
FROM empleado e
LEFT JOIN cliente cl
	ON e.codigo_empleado = cl.codigo_empleado_rep_ventas
LEFT JOIN (SELECT empleado.nombre AS nombre_empleado, jefe.nombre as nombre_del_jefe
	FROM empleado      
	LEFT JOIN empleado AS jefe
		ON jefe.codigo_empleado = empleado.codigo_jefe) AS t1
	ON e.nombre = t1.nombre_empleado
WHERE cl.nombre_cliente IS NULL;
...
SELECT	pais, COUNT(codigo_cliente) As numero_clientes
FROM cliente
GROUP BY pais;
...
SELECT MAX(total) AS precio_producto_mas_caro, MIN(total) AS precio_producto_mas_barato
FROM pago;
...
SELECT e.nombre, COUNT(cl.codigo_cliente) AS numero_clientes
FROM empleado e
LEFT JOIN cliente cl
	ON e.codigo_empleado = cl.codigo_empleado_rep_ventas
...
SELECT cl.nombre_cliente, cl.apellido_contacto, MIN(fecha_pago) AS primer_pago, MAX(fecha_pago) AS ultimo_pago
FROM cliente cl
INNER JOIN pago p
	ON cl.codigo_cliente = p.codigo_cliente
...
SELECT codigo_pedido, SUM(cantidad) AS cantidad_productos_pedidos
FROM detalle_pedido 
GROUP BY codigo_pedido;

-- 53. Devuelve un listado de los 20 productos más vendidos y el número total de unidades que se han vendido de cada uno. 
-- El listado deberá estar ordenado por el número total de unidades vendidas.
    
SELECT p.nombre, dp.codigo_producto, SUM(cantidad) AS cantidad_productos_pedidos
FROM detalle_pedido dp
INNER JOIN producto p
	ON dp.codigo_producto = p.codigo_producto
GROUP BY codigo_producto
ORDER BY cantidad_productos_pedidos DESC
LIMIT 20;
...
SELECT 
	...
	sum(cantidad*precio_unidad) + sum(cantidad*precio_unidad *0.21) AS total_facturado
FROM detalle_pedido;
...
SELECT p.nombre,
	...
	SUM(dt.cantidad)*precio_unidad AS total_facturado,
    SUM(dt.cantidad)*precio_unidad *1.21 AS total_con_impuestos
FROM detalle_pedido dt
INNER JOIN producto p
	ON dt.codigo_producto = p.codigo_producto
GROUP BY dt.codigo_producto
...
-- 61. Devuelve el nombre del producto del que se han vendido más unidades. 

SELECT DISTINCT p.nombre
FROM producto p
INNER JOIN detalle_pedido dp
ON p.codigo_producto = dp.codigo_producto
WHERE p.codigo_producto = (SELECT codigo_producto
	FROM detalle_pedido 
	GROUP BY codigo_producto
...
SELECT nombre_cliente, limite_credito,sum(total) AS suma_total_pagos
FROM cliente AS e, pago AS p
WHERE e.codigo_cliente = p.codigo_cliente
GROUP BY p.codigo_cliente
HAVING limite_credito > suma_total_pagos;
...
SELECT *
FROM cliente
WHERE codigo_cliente NOT IN (SELECT p.codigo_cliente
FROM pago p
INNER JOIN cliente cl
	ON p.codigo_cliente = cl.codigo_cliente);
    
-- 71. Devuelve un listado que muestre solamente los clientes que sí han realizado algún pago.
...
-- 73. Devuelve el nombre, apellidos, puesto y teléfono de la oficina de aquellos empleados que no sean representante de ventas de ningún cliente.

SELECT nombre, apellido1, puesto, o.telefono
FROM empleado e
LEFT JOIN oficina o
...
-- 74. Devuelve las oficinas donde no trabajan ninguno de los empleados que hayan sido los representantes de ventas de algún cliente que haya realizado la compra de algún producto de la gama Frutales.

SELECT o.* 
FROM oficina o
LEFT JOIN empleado e
	ON o.codigo_oficina = e.codigo_oficina
WHERE codigo_empleado NOT IN (SELECT codigo_empleado
	FROM empleado e
	INNER JOIN (SELECT t1.codigo_cliente, t1.codigo_empleado_rep_ventas, t1.codigo_pedido, t2.gama
		FROM (SELECT cl.codigo_cliente, cl.codigo_empleado_rep_ventas, p.codigo_pedido
			FROM cliente cl
			INNER JOIN pedido p
				ON 	cl.codigo_cliente = p.codigo_cliente) AS t1
	INNER JOIN (SELECT p.codigo_producto, p.gama, dp.codigo_pedido
		FROM producto p
		INNER JOIN detalle_pedido dp
			ON 	p.codigo_producto = dp.codigo_producto) AS t2
		ON t1.codigo_pedido = t2.codigo_pedido
	WHERE t2.gama = 'Frutales') AS t3
		ON e.codigo_empleado = t3.codigo_empleado_rep_ventas);

-- 75. Devuelve un listado con los clientes que han realizado algún pedido pero no han realizado ningún pago.		

SELECT DISTINCT cl.*
FROM cliente cl
INNER JOIN (SELECT t1.codigo_cliente
	FROM pago p
	RIGHT JOIN (SELECT cl.codigo_cliente
		FROM cliente cl
		INNER JOIN pedido p
			ON cl.codigo_cliente = p.codigo_cliente) AS t1
		ON p.codigo_cliente = t1.codigo_cliente
	WHERE total IS NULL) AS t2
	ON cl.codigo_cliente = t2.codigo_cliente;
    
-- 76. Devuelve un listado que muestre solamente los clientes que no han realizado ningún pago.
...
-- 77. Devuelve un listado de los productos que nunca han aparecido en un pedido.
...
-- 78. Devuelve un listado de los productos que han aparecido en un pedido alguna vez.
...
-- 79. Devuelve el listado de clientes indicando el nombre del cliente y cuántos pedidos ha realizado. Tenga en cuenta que pueden existir clientes que no han realizado ningún pedido.

SELECT nombre_cliente, count(codigo_pedido)	AS pedidos_realizados
FROM cliente cl
LEFT JOIN pedido p
	ON cl.codigo_cliente = p.codigo_cliente
group by cl.codigo_cliente;
...
-- 80. Devuelve un listado con los nombres de los clientes y el total pagado por cada uno de ellos. Tenga en cuenta que pueden existir clientes que no han realizado ningún pago.
...
-- 81. Devuelve el nombre de los clientes que hayan hecho pedidos en 2008 ordenados alfabéticamente de menor a mayor.
...
-- 82. Devuelve el nombre del cliente, el nombre y primer apellido de su representante de ventas 
...
SELECT t2.nombre_cliente, t2.nombre, t2.apellido1, o.telefono
FROM (SELECT t1.nombre_cliente, e.nombre, e.apellido1, codigo_oficina
	FROM (SELECT nombre_cliente, codigo_empleado_rep_ventas
		FROM cliente cl
		LEFT JOIN pago p
				ON cl.codigo_cliente = p.codigo_cliente
		WHERE id_transaccion IS NULL) AS t1
	INNER JOIN empleado e
		ON t1.codigo_empleado_rep_ventas = e.codigo_empleado) AS t2
INNER JOIN oficina o
	ON t2.codigo_oficina = o.codigo_oficina;
    
-- 83. Devuelve el listado de clientes donde aparezca el nombre del cliente, el nombre y primer apellido de su representante de ventas y la ciudad donde está su oficina.

SELECT cl.nombre_cliente, t1.codigo_empleado, t1.nombre, t1.apellido1, t1.ciudad
FROM (SELECT codigo_empleado, nombre, apellido1, ciudad
	FROM empleado e
	INNER JOIN oficina o
		ON e.codigo_oficina = o.codigo_oficina) AS t1
INNER JOIN cliente cl
	ON t1.codigo_empleado = cl.codigo_empleado_rep_ventas;
    
-- 84. Devuelve el nombre, apellidos, puesto y teléfono de la oficina de aquellos empleados que no sean representante de ventas de ningún cliente.

SELECT t1.nombre, t1.apellido1, t1.apellido2, o.telefono
FROM (SELECT nombre, apellido1, apellido2, codigo_oficina
	FROM cliente cl
	RIGHT JOIN empleado e
		ON cl.codigo_empleado_rep_ventas = e.codigo_empleado
	WHERE codigo_cliente IS NULL) AS t1
INNER JOIN oficina o
	ON t1.codigo_oficina = o.codigo_oficina;
    
-- 85. Devuelve un listado indicando todas las ciudades donde hay oficinas y el número de empleados que tiene.
